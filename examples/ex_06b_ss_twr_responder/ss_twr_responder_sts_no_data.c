 ...
 *
 *           A "packet" refers to a IEEE 802.15.4z STS Mode 3 frame that 
 *           contains no payload.
 *           A "frame" refers to a IEEE 802.15.4z STS Mode 0/1/2 frame that 
 *           contains a payload.
 *
 ...
 *
 *           In these examples (ss_twr_initiator_sts_no_data/ss_twr_responder_sts_no_data), 
 *           the initiator will send an SP3 mode "poll" packet to the responder
 *           while the initiator will save the TX timestamp of the "poll" 
 *           packet. The responder will await the "poll" packet from the 
 *           initiator and check that the STS quality is correct. If it is 
 *           correct, it will respond with a "resp" packet that is also in SP3 
 *           mode. The responder will save the RX and TX timestamps of the 
 *           packets. Finally, the initiator and responder will re-configure to
 *           send/receive SP0 packets. The responder will send a "report" frame
 *           to the initiator that contains the RX timestamp of the "poll" 
 *           packet and the TX timestamp of the "resp" packet.
 *
...
/* Length of the common part of the frame (up to and including 
 * the function code, see NOTE 3 below). */
...
int app_main(void)
{
    ...
    /* Next can enable TX/RX states output on GPIOs 5 and 6 to 
     * help diagnostics, and also TX/RX LEDs */
    ...
    while (1) {
        ...
        while (!((status_reg = dwt_read32bitreg(SYS_STATUS_ID)) & (SYS_STATUS_RXFR_BIT_MASK | 
                                                                   SYS_STATUS_ALL_RX_TO | 
                                                                   SYS_STATUS_ALL_ND_RX_ERR)))
        { /* spin */ };
        ...
        if (status_reg & SYS_STATUS_RXFR_BIT_MASK) {
            ...
            if (goodSts >= 0) {
                 ...
                 * and proceed to the next one. See NOTE 9 & 10 below. */
                if (ret == DWT_SUCCESS) {
                    ...
                    /* Set the delay to be twice the previous time period with 
                     * respect to the RX timestamp of the POLL packet. */
                    ...
                    if (ret == DWT_SUCCESS) {
                         ...
                         * of the report frame (modulo 256). */
                        frame_seq_nb++;
                    }
                }
                else {
