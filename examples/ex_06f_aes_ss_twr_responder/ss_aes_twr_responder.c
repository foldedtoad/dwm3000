 ...
 *
 *           This is a simple code example which acts as the responder in a SS 
 ...
 *           simple 802.15.4 standard data frame. This example is getting an 
 *           encrypted data, its response will be encrypted also.
 *           This application waits for a "poll" message (recording the RX 
 *           time-stamp of the poll) expected from the "SS TWR initiator"
 *           example code (companion to this application), and then sends a 
 *           "response" message to complete the exchange.
 *           The response message contains all the time-stamps recorded by 
 *           this application, including the calculated/predicted TX 
 *           time-stamp for the response message itself. The companion "SS TWR 
 *           initiator" example application works out the time-of-flight 
...
/* This SS-TWR example will use sample MAC data frame format as defined 
 * by mac_frame_802_15_4_format_t structure */
 ...
 * be calibrated prior to taking reference measurements.
 * See NOTE 5 below. */
    ...
    while (1) {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) {
            ...
            if (memcmp(rx_buffer, rx_poll_msg, aes_job_rx.payload_len) == 0) {
                ...
                /* perform the encryption, the TX buffer will contain a 
                 * full MAC frame with encrypted payload */
                 ...
                 * and proceed to the next one. See NOTE 10 below. */
                if (ret == DWT_SUCCESS) {
