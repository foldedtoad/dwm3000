 ...
 *
 *           This is a simple code example which acts as the initiator in a SS 
 *           TWR distance measurement exchange. In this example, data including 
 *           time signature will be sent encrypted. This example is using a 
 *           simple 802.15.4 standard data frame. This example is sending an 
 *           encrypted data, it assumes that ss_AES_twr_resp.c data will be 
 *           encrypted also. This application sends a "poll" frame (recording 
 *           the TX time-stamp of the poll), after which it waits for a 
 *           "response" message from the "DS TWR responder" example code 
 *           (companion to this application) to complete the exchange. 
 *           The response message contains the remote responder's time-stamps 
 *           of poll RX, and response TX. With this data and the local 
 *           time-stamps, (of poll TX and response RX), this example application
 *           works out a value for the time-of-flight over-the-air and, thus, 
 *           the estimated distance between the two devices, which it 
...
LOG_MODULE_REGISTER(ss_aes_twr_init);

/* Example application name */
#define APP_NAME "SS AES TWR INIT v1.0"

/* Sample of 802_15_4 frame*/
mac_frame_802_15_4_format_t     mac_frame = {
    /*
    * Frame control[0] = 0x09 = Data frame, security enabled, PEND not set, 
    *    no ACK required, PANID compression set to zero (no PANID for source)
    * Frame control[1] = 0xEC = With seq num, no IEs, using extended address, 
    *    frame ver 2 (IEEE Std 802.15.4)
    */
...
int app_main(void)
{
    ...
    while (1) {
        ...
        /* The AES job will take the TX frame data and and copy it to 
         * DW IC TX buffer before transmission. See NOTE 7 below. */
        status = dwt_do_aes(&aes_job_tx,aes_config.aes_core_type);

        /* Check for errors */
        if (status < 0) {
            LOG_ERR("AES length error");
            while (1) { /* spin */ };
        }
         ...
         * is enabled automatically after the frame is sent and the delay
        ...
        /* Increment frame sequence number (modulo 256) and frame counter, 
         * after transmission of the poll message . */

        MAC_FRAME_SEQ_NUM_802_15_4(&mac_frame) = ++seq_cnt;
        mac_frame_update_aux_frame_cnt(&mac_frame, ++frame_cnt);

        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) {  /* Got response */
